...
var enableSelectiveHydration = true; // Flight experiments
...
var disableJavaScriptURLs = false; // These APIs will no longer be "unstable" in the upcoming 16.7 release,
...
var preventIntermediateStates = true; // --------------------------
...
var disableLegacyContext = false; // Disables React.createFactory

 // Disables children for <textarea> elements

var disableTextareaChildren = false; // Disables ReactDOM.unstable_renderSubtreeIntoContainer

var disableUnstableRenderSubtreeIntoContainer = false; // Disables ReactDOM.unstable_createPortal

var disableUnstableCreatePortal = false;
...
function validateEventValue(eventValue) {
  if (typeof eventValue === 'object' && eventValue !== null) {
    ...
    var showWarning = function (name) {
      {
        error('%s is not available on event objects created from event responder modules (React Flare). ' + 'Try wrapping in a conditional, i.e. `if (event.type !== "press") { event.%s }`', name, name);
      }
    };
...
function traverseAndHandleEventResponderInstances(topLevelType, targetFiber, nativeEvent, nativeEventTarget, eventSystemFlags) {
  ...
  var responderEvent = createDOMResponderEvent(topLevelType, nativeEvent, nativeEventTarget, isPassiveEvent);
  ...
  if (passive !== undefined) {
    rootEventResponderInstances.push.apply(rootEventResponderInstances, Array.from(passive));
  }

  if (!isPassive) {
    var active = rootEventTypesToEventResponderInstances.get(topLevelType + '_active');

    if (active !== undefined) {
      rootEventResponderInstances.push.apply(rootEventResponderInstances, Array.from(active));
    }
  }
...
function DEPRECATED_dispatchEventForResponderEventSystem(topLevelType, targetFiber, nativeEvent, nativeEventTarget, eventSystemFlags) {
  ...
  if (node.namespaceURI === Namespaces.svg) {
    {
      if (enableTrustedTypesIntegration) {
        ...
        if (typeof trustedTypes !== 'undefined') {
          error("Using 'dangerouslySetInnerHTML' in an svg element with " + 'Trusted Types enabled in an Internet Explorer will cause ' + 'the trusted value to be converted to string. Assigning string ' + "to 'innerHTML' will throw an error if Trusted Types are enforced. " + "You can try to wrap your svg element inside a div and use 'dangerouslySetInnerHTML' " + 'on the enclosing div instead.');
        }
      }
    }
...
function trapReplayableEvent(topLevelType, document, listenerMap) {
  listenToTopLevel(topLevelType, document, listenerMap);

  if (enableDeprecatedFlareAPI) {
    // Trap events for the responder system.
    var topLevelTypeString = unsafeCastDOMTopLevelTypeToString(topLevelType);
    var passiveEventKey = topLevelTypeString + '_passive';

    if (!listenerMap.has(passiveEventKey)) {
      var listener = addResponderEventSystemEvent(document, topLevelTypeString, true);
...
function eagerlyTrapReplayableEvents(document) {
  var listenerMap = getListenerMapForElement(document); // Discrete
...
var eventPriorities = new Map(); // We store all the DOMTopLevelEventTypes and their React Types in pairs of two.
// Furthermore, we ignore prettier so we can keep the formatting sane.
// prettier-ignore

var discreteEvents = [TOP_BLUR, 'blur', TOP_CANCEL, 'cancel', TOP_CLICK, 'click', TOP_CLOSE, 'close', TOP_CONTEXT_MENU, 'contextMenu', TOP_COPY, 'copy', TOP_CUT, 'cut', TOP_AUX_CLICK, 'auxClick', TOP_DOUBLE_CLICK, 'doubleClick', TOP_DRAG_END, 'dragEnd', TOP_DRAG_START, 'dragStart', TOP_DROP, 'drop', TOP_FOCUS, 'focus', TOP_INPUT, 'input', TOP_INVALID, 'invalid', TOP_KEY_DOWN, 'keyDown', TOP_KEY_PRESS, 'keyPress', TOP_KEY_UP, 'keyUp', TOP_MOUSE_DOWN, 'mouseDown', TOP_MOUSE_UP, 'mouseUp', TOP_PASTE, 'paste', TOP_PAUSE, 'pause', TOP_PLAY, 'play', TOP_POINTER_CANCEL, 'pointerCancel', TOP_POINTER_DOWN, 'pointerDown', TOP_POINTER_UP, 'pointerUp', TOP_RATE_CHANGE, 'rateChange', TOP_RESET, 'reset', TOP_SEEKED, 'seeked', TOP_SUBMIT, 'submit', TOP_TOUCH_CANCEL, 'touchCancel', TOP_TOUCH_END, 'touchEnd', TOP_TOUCH_START, 'touchStart', TOP_VOLUME_CHANGE, 'volumeChange']; // prettier-ignore

var userBlockingEvents = [TOP_DRAG, 'drag', TOP_DRAG_ENTER, 'dragEnter', TOP_DRAG_EXIT, 'dragExit', TOP_DRAG_LEAVE, 'dragLeave', TOP_DRAG_OVER, 'dragOver', TOP_MOUSE_MOVE, 'mouseMove', TOP_MOUSE_OUT, 'mouseOut', TOP_MOUSE_OVER, 'mouseOver', TOP_POINTER_MOVE, 'pointerMove', TOP_POINTER_OUT, 'pointerOut', TOP_POINTER_OVER, 'pointerOver', TOP_SCROLL, 'scroll', TOP_TOGGLE, 'toggle', TOP_TOUCH_MOVE, 'touchMove', TOP_WHEEL, 'wheel']; // prettier-ignore

var continuousEvents = [TOP_ABORT, 'abort', TOP_ANIMATION_END, 'animationEnd', TOP_ANIMATION_ITERATION, 'animationIteration', TOP_ANIMATION_START, 'animationStart', TOP_CAN_PLAY, 'canPlay', TOP_CAN_PLAY_THROUGH, 'canPlayThrough', TOP_DURATION_CHANGE, 'durationChange', TOP_EMPTIED, 'emptied', TOP_ENCRYPTED, 'encrypted', TOP_ENDED, 'ended', TOP_ERROR, 'error', TOP_GOT_POINTER_CAPTURE, 'gotPointerCapture', TOP_LOAD, 'load', TOP_LOADED_DATA, 'loadedData', TOP_LOADED_METADATA, 'loadedMetadata', TOP_LOAD_START, 'loadStart', TOP_LOST_POINTER_CAPTURE, 'lostPointerCapture', TOP_PLAYING, 'playing', TOP_PROGRESS, 'progress', TOP_SEEKING, 'seeking', TOP_STALLED, 'stalled', TOP_SUSPEND, 'suspend', TOP_TIME_UPDATE, 'timeUpdate', TOP_TRANSITION_END, 'transitionEnd', TOP_WAITING, 'waiting'];
/**
 * Turns
 * ['abort', ...]
 * into
...
function processTopEventTypesByPriority(eventTypes, priority) {
...
function removeActiveResponderEventSystemEvent(document, topLevelType, listener) {
  if (passiveBrowserEventsSupported) {
...
function listenTo(registrationName, mountAt) {
  var listeningSet = getListenerMapForElement(mountAt);
...
function listenToTopLevel(topLevelType, mountAt, listenerMap) {
...
var STYLE$1 = 'style';
var HTML = '__html';
var DEPRECATED_flareListeners = 'DEPRECATED_flareListeners';
...
function setInitialDOMProperties(tag, domElement, rootContainerElement, nextProps, isCustomComponentTag) {
  for (var propKey in nextProps) {
    ...
    } else if (enableDeprecatedFlareAPI && propKey === DEPRECATED_flareListeners || propKey === SUPPRESS_CONTENT_EDITABLE_WARNING || propKey === SUPPRESS_HYDRATION_WARNING$1) {// Noop
    ...
    if (type === 'script') {
...

      {
        if (enableTrustedTypesIntegration && !didWarnScriptTags) {
          error('Encountered a script tag while rendering React component. ' + 'Scripts inside React components are never executed when rendering ' + 'on the client. Consider using template tag instead ' + '(https://developer.mozilla.org/en-US/docs/Web/HTML/Element/template).');
...
function diffHydratedProperties(domElement, tag, rawProps, parentNamespace, rootContainerElement) {
  ...
  for (var propKey in rawProps) {
    ...
    typeof isCustomComponentTag === 'boolean') {
      ...
      } else if (enableDeprecatedFlareAPI && propKey === DEPRECATED_flareListeners || propKey === SUPPRESS_CONTENT_EDITABLE_WARNING || propKey === SUPPRESS_HYDRATION_WARNING$1 || // Controlled attributes are not validated
      // TODO: Only ignore them on controlled tags.
...
function listenToEventResponderEventTypes(eventTypes, document) {
  if (enableDeprecatedFlareAPI) {
    ...
    var listenerMap = getListenerMapForElement(document); // Go through each target event type of the event responder
    ...
    for (var i = 0, length = eventTypes.length; i < length; ++i) {
      ...
      if (!listenerMap.has(eventKey)) {
        if (isPassive) {
          var activeKey = targetEventType + '_active'; // If we have an active event listener, do not register
          ...
          var passiveKey = targetEventType + '_passive';
          var passiveListener = listenerMap.get(passiveKey);
        ...
        var eventListener = addResponderEventSystemEvent(document, targetEventType, isPassive);
...
function resetAfterCommit(containerInfo) {
  ...
  if (enableDeprecatedFlareAPI) {
    var activeElementDetached = selectionInformation.activeElementDetached;
...
function DEPRECATED_mountResponderInstance(responder, responderInstance, responderProps, responderState, instance) {
  // Listen to events
...
function DEPRECATED_unmountResponderInstance(responderInstance) {
  if (enableDeprecatedFlareAPI) {
...
function computeSuspenseTimeout(currentTime, timeoutMs) {
  var currentTimeMs = expirationTimeToMs(currentTime);
  var deadlineMs = currentTimeMs + timeoutMs;
      ...
      while (dependency !== null) {
        ...
        if (dependency.context === context && (dependency.observedBits & changedBits) !== 0) {
          ...
          if (fiber.tag === ClassComponent) {
            ...
            var update = createUpdate(NoWork, renderExpirationTime, null);
...
function createUpdate(eventTime, expirationTime, suspenseConfig) {
  var update = {
    eventTime: eventTime,
    ...
    if (first !== null) {
      ...
      do {
        ...
        if (updateExpirationTime < renderExpirationTime) {
          ...
          markRenderEventTimeAndConfig(eventTime, update.suspenseConfig); // Process this update.
...
function requestCurrentSuspenseConfig() {
  return ReactCurrentBatchConfig$1.suspense;
}
...
function updateEventListener(listener, fiber, visistedResponders, respondersMap, rootContainerInstance) {
  ...
  if (visistedResponders.has(responder)) {
    // show warning
    {
      error('Duplicate event responder "%s" found in event listeners. ' + 'Event listeners passed to elements cannot use the same event responder more than once.', responder.displayName);
    }
...
function updateDeprecatedEventListeners(listeners, fiber, rootContainerInstance) {
...
function renderWithHooks(current, workInProgress, Component, props, secondArg, nextRenderExpirationTime) {
...

  {
    if (current !== null && current.memoizedState !== null) {
      ReactCurrentDispatcher$2.current = HooksDispatcherOnUpdateInDEV;
      ...
      ReactCurrentDispatcher$2.current = HooksDispatcherOnMountWithHookTypesInDEV;
    } else {
      ReactCurrentDispatcher$2.current = HooksDispatcherOnMountInDEV;
    }
  }
  ...
  if (workInProgress.expirationTime === renderExpirationTime$1) {
    ...
    do {
      ...
      ReactCurrentDispatcher$2.current = HooksDispatcherOnRerenderInDEV;
...
function renderPhaseDispatch(fiber, queue, action) {
  if (currentlyRenderingFiber$1 === fiber || currentlyRenderingFiber$1 !== null && currentlyRenderingFiber$1.alternate === fiber) {
...
function mountReducer(reducer, initialArg, init) {
  ...
  var boundDispatch = queue.dispatch = dispatch.bind(null, currentlyRenderingFiber$1, queue);
...
function updateReducer(reducer, initialArg, init) {
  ...
  if (baseQueue !== null) {
    ...
    do {
      ...
      if (updateExpirationTime < renderExpirationTime$1) {
        ...
        markRenderEventTimeAndConfig(eventTime, suspenseConfig); // Process this update.
  ...
  var dispatchMethod = queue.dispatch;
...
function mountState(initialState) {
  ...
  var dispatchMethod = queue.dispatch = dispatch.bind(null, currentlyRenderingFiber$1, queue);
  return [hook.memoizedState, dispatchMethod];
}
...
function mountDeferredValue(value, config) {
  ...
  mountEffect(function () {
    var previousConfig = ReactCurrentBatchConfig$2.suspense;
    ReactCurrentBatchConfig$2.suspense = config === undefined ? null : config;

    try {
      setValue(value);
    } finally {
      ReactCurrentBatchConfig$2.suspense = previousConfig;
    }
  }, [value, config]);
  return prevValue;
}
...
function mountTransition(config) {
  ...
  hook.queue = instance;
  var isPending = false; // TODO: Consider passing `config` to `startTransition` instead of the hook.
  ...
  var start = mountCallback(startTransition.bind(null, instance, config), [config]);
  hook.memoizedState = {
    isPending: isPending,
    version: 0
  };
...
function updateTransition(config) {
  var hook = updateWorkInProgressHook();
  var oldState = hook.memoizedState;
  var oldVersion = oldState.version;
  var instance = hook.queue;
  var newVersion = instance.version; // Check if the most recent transition is pending. The following logic is a
  ...
  if (oldVersion === newVersion) {
    ...
    var pendingTime = instance.pendingTime;
    var resolvedTime = instance.resolvedTime;
    var oldIsPending = oldState.isPending;
    var remainingExpirationTime;
    var memoizedVersion;

    if (renderExpirationTime$1 <= resolvedTime) {
      // Transition has finished.
      newIsPending = false;
      remainingExpirationTime = NoWork; // Only update the memoized version number once the transition finishes.

      memoizedVersion = newVersion;
    } else if (renderExpirationTime$1 <= pendingTime) {
    ...
    if (remainingExpirationTime > currentlyRenderingFiber$1.expirationTime) {
      // Mark that there's remaining work
      currentlyRenderingFiber$1.expirationTime = remainingExpirationTime;
  ...
  var start = updateCallback(startTransition.bind(null, instance, config), [config]);
  return [start, newIsPending];
}

function rerenderTransition(config) {
  var hook = updateWorkInProgressHook();
  var isPending = hook.memoizedState.isPending;
  var instance = hook.queue;
  var start = updateCallback(startTransition.bind(null, instance, config), [config]);
  return [start, isPending];
}
...
function setInvalidNestedHooksDispatcher() {
  {
    var prevDispatcher = ReactCurrentDispatcher$2.current;
    ReactCurrentDispatcher$2.current = InvalidNestedHooksDispatcherOnUpdateInDEV;
...

{
  ...
  HooksDispatcherOnMountInDEV = {
    ...
    useMemo: function (create, deps) {
      ...
      checkDepsAreArrayDev(deps);
      var prevDispatcher = ReactCurrentDispatcher$2.current;
      ReactCurrentDispatcher$2.current = InvalidNestedHooksDispatcherOnMountInDEV;
  ...
  HooksDispatcherOnRerenderInDEV = {
    ...
    useMemo: function (create, deps) {
      ...
      updateHookTypesDev();
      var prevDispatcher = ReactCurrentDispatcher$2.current;
      ReactCurrentDispatcher$2.current = InvalidNestedHooksDispatcherOnRerenderInDEV;
...
var currentTransition = null; // The event time of the current transition.

var currentTransitionEventTime = NoWork; // Inside `startTransition`, this is the expiration time of the update that
// turns on `isPending`. We also use it to turn off the `isPending` of previous
// transitions, if they exists.

var currentTransitionPendingTime = NoWork; // The expiration time of the current transition.

var currentTransitionResolvedTime = NoWork; // The expiration time of the transitions that were superseded by the current
// transition. This is accumulated during `startTransition`.

var currentTransitionSupersededTime = NoWork;
...
function dispatch(fiber, queue, action) {
  ...
  if (renderPhaseDispatch(fiber, queue, action)) {
    return;
  }
  ...
  if (currentTransition !== null) {
    eventTime = currentTransitionEventTime;
    expirationTime = currentTransitionResolvedTime;
    setTransition(queue, currentTransition);
  } else {
    eventTime = requestCurrentTimeForUpdate();
    expirationTime = computeExpirationForFiber(eventTime, fiber, suspenseConfig);
  }
  ...
  if (fiber.expirationTime === NoWork && (alternate === null || alternate.expirationTime === NoWork)) {
    ...
    if (lastRenderedReducer !== null) {
      var prevDispatcher = setInvalidNestedHooksDispatcher();
      ...
      } catch (error$$1) {// Suppress the error. It will throw again in the render phase.
      } finally {
        {
...
var classComponentUpdater = {
  ...
  enqueueSetState: function (inst, payload, callback) {
    ...
    if (currentTransition !== null) {
      ...
      expirationTime = currentTransitionResolvedTime;
      var updateQueue = fiber.updateQueue;
      var sharedQueue = updateQueue !== null ? updateQueue.shared : null;
    ...
    var update = createUpdate(eventTime, expirationTime, suspenseConfig);
...
function startTransition(transitionInstance, config, callback) {
  var suspenseConfig = config === undefined ? null : config;
  var fiber = transitionInstance.fiber; // Don't need to reset these at the end because they're impossible to read
  // from outside of a `startTransition` callback.

  currentTransitionEventTime = requestCurrentTimeForUpdate(); // TODO: runWithPriority shouldn't be necessary here. React should manage its
  // own concept of priority, and only consult Scheduler for updates that are
  // scheduled from outside a React context.

  var priorityLevel = getCurrentPriorityLevel();
  runWithPriority$2(priorityLevel < UserBlockingPriority$2 ? UserBlockingPriority$2 : priorityLevel, function () {
    currentTransitionPendingTime = computeExpirationForFiber(currentTransitionEventTime, fiber, null);
  });
  var root = scheduleUpdateOnFiber(fiber, currentTransitionPendingTime);
  runWithPriority$2(priorityLevel > NormalPriority ? NormalPriority : priorityLevel, function () {
    var previousConfig = ReactCurrentBatchConfig.suspense;
    var previousTransition = currentTransition;
    ReactCurrentBatchConfig.suspense = suspenseConfig;
    currentTransition = transitionInstance;
    currentTransitionResolvedTime = computeExpirationForFiber(currentTransitionEventTime, fiber, suspenseConfig);
    ...
    try {
      ...
      if (preventIntermediateStates && currentTransitionSupersededTime !== NoWork && root !== null) {
        markTransitionRange(root, currentTransitionSupersededTime, currentTransitionResolvedTime);
      }
...
function markTransitionRange(root, start, end) {
  ...
  var pendingRanges = root.pendingRanges;
  ...
  if (pendingRanges === null) {
    ...
    // should ever overlap.
    for (var i = 0; i < pendingRanges.length;) {
      var start2 = pendingRanges[i];
      var end2 = pendingRanges[i + 1];

      if (start >= end2 && end <= start2) {
        // Found an overlapping range. Combine them.
        start = start > start2 ? start : start2;
        end = end < end2 ? end : end2; // There could be multiple overlapping ranges. Remove this one and keep
        // iterating. We'll add a single, combined range at the end.

        pendingRanges.splice(i, 2); // Continue searching for overlapping ranges. We don't need to check the
...
function setTransition(queue, transition) {
  var prevTransition = queue.pendingTransition;
  ...
  if (transition !== prevTransition) {
    ...
    if (prevTransition !== null) {
      ...
      if (preventIntermediateStates) {
        var resolvedTime = prevTransition.resolvedTime;
...
function ChildReconciler(shouldTrackSideEffects) {
  ...
  function updateElement(returnFiber, current$$1, element, expirationTime) {
    if (current$$1 !== null) {
      if (current$$1.elementType === element.type || ( // Keep this check inline so it only runs on the false path:
      isCompatibleFamilyForHotReloading(current$$1, element))) {
        ...
        return existing;
      } else if (enableChunksAPI && current$$1.tag === Chunk && element.type.$$typeof === REACT_CHUNK_TYPE && element.type.render === current$$1.type.render) {
        // Same as above but also update the .type field.
        var _existing = useFiber(current$$1, element.props, expirationTime);
    ...
    while (child !== null) {
      ...
      if (child.key === key) {
        switch (child.tag) {
          case Fragment:
            {
              if (element.type === REACT_FRAGMENT_TYPE) {
                ...
                var existing = useFiber(child, element.props.children, expirationTime);
                existing.return = returnFiber;
          ...
          case Chunk:
            if (enableChunksAPI) {
              if (element.type.$$typeof === REACT_CHUNK_TYPE && element.type.render === child.type.render) {
                deleteRemainingChildren(returnFiber, child.sibling);

                var _existing2 = useFiber(child, element.props, expirationTime);
          ...
          default:
            {
              ...
              isCompatibleFamilyForHotReloading(child, element))) {
                ...
                var _existing3 = useFiber(child, element.props, expirationTime);
...
function updateChunk(current$$1, workInProgress, chunk, nextProps, renderExpirationTime) {
  // TODO: current can be non-null here even if the component
  // hasn't yet mounted. This happens after the first render suspends.
  // We'll need to figure out if this is fine or can cause issues.
  var render = chunk.render;
  var data = chunk.query(); // The rest is a fork of updateFunctionComponent
...

  {
    ReactCurrentOwner$3.current = workInProgress;
    setCurrentPhase('render');
    nextChildren = renderWithHooks(current$$1, workInProgress, render, nextProps, data, renderExpirationTime);
...
function updateHostRoot(current$$1, workInProgress, renderExpirationTime) {
  ...
  if (!(current$$1 !== null && updateQueue !== null)) {
    {
    ...
    case Chunk:
      {
        if (enableChunksAPI) {
          ...
          child = updateChunk(null, workInProgress, Component, props, renderExpirationTime);
          return child;
        }
  ...
  if (typeof value === 'object' && value !== null && typeof value.render === 'function' && value.$$typeof === undefined) {
...

    {
      if (disableLegacyContext && Component.contextTypes) {
        error('%s uses the legacy contextTypes API which is no longer supported. ' + 'Use React.createContext() with React.useContext() instead.', getComponentName(Component) || 'Unknown');
      }
...
function beginWork$1(current$$1, workInProgress, renderExpirationTime) {
  ...
  switch (workInProgress.tag) {
    ...
    case Chunk:
      {
        if (enableChunksAPI) {
          var chunk = workInProgress.type;
          var props = workInProgress.pendingProps;
          return updateChunk(current$$1, workInProgress, chunk, props, renderExpirationTime);
        }
...
function collectScopedNodes(node, fn, scopedNodes) {
  if (enableScopeAPI) {
    if (node.tag === HostComponent) {
      ...
      if (instance !== null && fn(type, memoizedProps || emptyObject$1, instance) === true) {
    ...
    case HostComponent:
      {
        ...
        if (current !== null && workInProgress.stateNode != null) {
          ...
          if (enableDeprecatedFlareAPI) {
            var prevListeners = current.memoizedProps.DEPRECATED_flareListeners;
            var nextListeners = newProps.DEPRECATED_flareListeners;
          ...
          if (_wasHydrated) {
            ...
            if (enableDeprecatedFlareAPI) {
              var listeners = newProps.DEPRECATED_flareListeners;
            ...
            if (enableDeprecatedFlareAPI) {
              var _listeners = newProps.DEPRECATED_flareListeners;

              if (_listeners != null) {
                updateDeprecatedEventListeners(_listeners, workInProgress, rootContainerInstance);
              }
    ...
    case ScopeComponent:
      {
        if (enableScopeAPI) {
          if (current === null) {
            ...
            if (enableDeprecatedFlareAPI) {
              var _listeners2 = newProps.DEPRECATED_flareListeners;

              if (_listeners2 != null) {
                var _rootContainerInstance2 = getRootHostContainer();

                updateDeprecatedEventListeners(_listeners2, workInProgress, _rootContainerInstance2);
              }
            }
            ...
            if (enableDeprecatedFlareAPI) {
              var _prevListeners = current.memoizedProps.DEPRECATED_flareListeners;
              var _nextListeners = newProps.DEPRECATED_flareListeners;
...
function safelyCallDestroy(current$$1, destroy) {
  {
    ...
    if (hasCaughtError()) {
      ...
      captureCommitPhaseError(current$$1, error$$1);
    }
  }
}
...
function commitUnmount(finishedRoot, current$$1, renderPriorityLevel) {
  ...
  switch (current$$1.tag) {
    ...
    case HostComponent:
      {
        if (enableDeprecatedFlareAPI) {
          unmountDeprecatedResponderListeners(current$$1);
          beforeRemoveInstance(current$$1.stateNode);
        }
    ...
    case HostComponent:
      {
        ...
        if (instance != null) {
          ...
          if (enableDeprecatedFlareAPI) {
            var prevListeners = oldProps.DEPRECATED_flareListeners;
            var nextListeners = newProps.DEPRECATED_flareListeners;
    ...
    case ScopeComponent:
      {
        if (enableScopeAPI) {
          ...
          if (enableDeprecatedFlareAPI) {
            ...
            var _nextListeners = _newProps.DEPRECATED_flareListeners;

            if (_prevListeners !== _nextListeners || current$$1 === null) {
              updateDeprecatedEventListeners(_nextListeners, finishedWork, null);
            }
          }
...
function createRootErrorUpdate(fiber, errorInfo, expirationTime) {
  var update = createUpdate(NoWork, expirationTime, null); // Unmount the root by rendering null.
...
function createClassErrorUpdate(fiber, errorInfo, expirationTime) {
  var update = createUpdate(NoWork, expirationTime, null);
        ...
        if ((_workInProgress.mode & BlockingMode) === NoMode) {
          ...
          if (sourceFiber.tag === ClassComponent) {
            ...
            if (currentSourceFiber === null) {
              ...
              // prevent a bail out.
              var update = createUpdate(NoWork, Sync, null);
              update.tag = ForceUpdate;
...
var workInProgressRootLatestProcessedEventTime = Sync;
    ...
    case RootSuspended:
      {
        ...
        var hasNotProcessedNewUpdates = workInProgressRootLatestProcessedEventTime === Sync;
    ...
    case RootSuspendedWithDelay:
      {
        ...
        !(true && flushSuspenseFallbacksInTests && IsThisRendererActing.current)) {
          ...
          if (workInProgressRootLatestSuspenseTimeout !== Sync) {
            ...
            var eventTimeMs = expirationTimeToMs(workInProgressRootLatestProcessedEventTime);
    ...
    case RootCompleted:
      {
        ...
        if ( // do not delay if we're inside an act() scope
        !(true && flushSuspenseFallbacksInTests && IsThisRendererActing.current) && workInProgressRootLatestProcessedEventTime !== Sync && workInProgressRootCanSuspendUsingConfig !== null) {
          // If we have exceeded the minimum loading delay, which probably
          // means we have shown a spinner already, we might have to suspend
          // a bit longer to ensure that the spinner is shown for
          // enough time.
          var _msUntilTimeout2 = computeMsUntilSuspenseLoadingDelay(workInProgressRootLatestProcessedEventTime, expirationTime, workInProgressRootCanSuspendUsingConfig);
...
function markRenderEventTimeAndConfig(eventTime, suspenseConfig) {
  ...
  if (eventTime > Idle) {
    ...
    if (suspenseConfig !== null) {
      ...
      var timeoutMs = suspenseConfig.timeoutMs | 0 || LOW_PRIORITY_EXPIRATION;
      var timeoutTime = computeSuspenseTimeout(eventTime, timeoutMs);
    ...
    do {
      {
        ...
        if (hasCaughtError()) {
          ...
          var error$$1 = clearCaughtError();
          captureCommitPhaseError(nextEffect, error$$1);
  ...
  while (effect !== null) {
    {
      ...
      if (hasCaughtError()) {
        ...
        var error$$1 = clearCaughtError();
        captureCommitPhaseError(effect, error$$1);
      }
...
function captureCommitPhaseError(sourceFiber, error$$1) {
...
function pingSuspendedRoot(root, thenable, suspendedTime) {
  ...
  if (workInProgressRoot === root && renderExpirationTime === suspendedTime) {
    ...
    if (workInProgressRootExitStatus === RootSuspendedWithDelay || workInProgressRootExitStatus === RootSuspended && workInProgressRootLatestProcessedEventTime === Sync && now() - globalMostRecentFallbackTime < FALLBACK_THROTTLE_MS) {
...
function computeMsUntilSuspenseLoadingDelay(mostRecentEventTime, committedExpirationTime, suspenseConfig) {
  ...
  var eventTimeMs = expirationTimeToMs(mostRecentEventTime);
...
function warnAboutUpdateOnUnmountedFiberInDEV(fiber) {
  {
    ...
    if (tag !== HostRoot && tag !== ClassComponent && tag !== FunctionComponent && tag !== ForwardRef && tag !== MemoComponent && tag !== SimpleMemoComponent && tag !== Chunk) {
...
function warnIfUnmockedScheduler(fiber) {
  {
    if (didWarnAboutUnmockedScheduler === false && Scheduler.unstable_flushAllWithoutAsserting === undefined) {
      if (fiber.mode & BlockingMode || fiber.mode & ConcurrentMode) {
        ...
        error('Starting from React v17, the "scheduler" module will need to be mocked ' + 'to guarantee consistent behaviour across tests and browsers. ' + 'For example, with jest: \n' + "jest.mock('scheduler', () => require('scheduler/unstable_mock'));\n\n" + 'For more info, visit https://fb.me/react-mock-scheduler');
      }
    }
  }
}
        ...
        if (current$$1 !== null) {
          ...
          switch (workInProgressNode.tag) {
            case ClassComponent:
              ...
              if (updateQueue !== null) {
                var update = updateQueue.baseQueue;
            ...
            case Chunk:
              {
                var firstHook = current$$1.memoizedState; // TODO: This just checks the first Hook. Isn't it suppose to check all Hooks?

                if (firstHook !== null && firstHook.baseQueue !== null) {
                  var _update = firstHook.baseQueue; // Loop through the functional component's memoized state to see whether
                  ...
                  while (_update !== null) {
                    ...
                    if (_update.next === firstHook.baseQueue) {
                      break;
                    }
...
function getNextRootExpirationTimeToWorkOn(root) {
  ...
  // received a ping.
  var lastExpiredTime = resolveTransitionTime(root, root.lastExpiredTime);
  ...
  var firstPendingTime = resolveTransitionTime(root, root.firstPendingTime);
  var firstSuspendedTime = root.firstSuspendedTime;
  var lastSuspendedTime = root.lastSuspendedTime;

  if (!(firstSuspendedTime !== NoWork && firstSuspendedTime >= firstPendingTime && lastSuspendedTime <= firstPendingTime)) {
  ...
  var nextLevel = resolveTransitionTime(root, lastPingedTime > nextKnownPendingLevel ? lastPingedTime : nextKnownPendingLevel);
...
function resolveTransitionTime(root, expirationTime) {
  ...
  if (pendingRanges !== null) {
    ...
    // not overlap.
    var resolvedTime = expirationTime;

    for (var i = 0; i < pendingRanges.length; i += 2) {
      var start = pendingRanges[i];
      var end = pendingRanges[i + 1];
...
function markRootPingedAtTime(root, expirationTime) {
  var lastPingedTime = root.lastPingedTime;
  ...
  if (enableTrainModelFix) {
    ...
    if (expirationTime <= Idle) {
      ...
      if (lastPingedTime === NoWork || lastPingedTime <= Idle && lastPingedTime > expirationTime) {
        // There are only Idle pings.
        root.lastPingedTime = expirationTime;
      }
    } else {
      // This is a non-Idle ping.
      if (lastPingedTime === NoWork || lastPingedTime > expirationTime || lastPingedTime <= Idle) {
...
function updateContainer(element, container, parentComponent, callback) {
  ...
  var update = createUpdate(currentTime, expirationTime, suspenseConfig); // Caution: React DevTools currently depends on this property
  // being called "element".
...
var didWarnAboutUnstableRenderSubtreeIntoContainer = false;
  ...
  ReactDOM.unstable_renderSubtreeIntoContainer = function () {
    {
      if (!didWarnAboutUnstableRenderSubtreeIntoContainer) {
        ...
        warn('ReactDOM.unstable_renderSubtreeIntoContainer() is deprecated ' + 'and will be removed in a future major release. Consider using ' + 'React Portals instead.');
      }
    }
